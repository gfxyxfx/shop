...
<head>
	...
	<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/vendor/webuploader/webuploader.css">
</head>
<body class="blank">
...
<script src="${pageContext.request.contextPath}/resources/vendor/bootstrap-fileinput-master/js/fileinput.js" type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/resources/vendor/bootstrap-fileinput-master/js/fileinput_locale_zh.js" type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/resources/vendor/webuploader/webuploader.js" type="text/javascript"></script>
<div id="wrapper">
					         			...
					         			<form:form modelAttribute="product" id="form_productInfo" name="form_productInfo" method="POST">
											...
											<div class="row" style="padding:20px">
		                                       	<div class="row">
			                                        <div class="form-group col-lg-6">
			                                            ...
			                                            <input name="name" class="form-control" type="text" placeholder="商品名称" value="${product.name}">
			                                        </div>
			                                        <div class="form-group col-lg-6">
			                                            <label>商品外部编号</label>
			                                            <input name="externalId" class="form-control" type="text" placeholder="商品外部编号" value="${product.externalId}">
			                                        </div>
		                                        </div>
		                                        <div class="row">
			                                        <div class="form-group col-lg-6">
			                                        ...
			                                        <div class="form-group col-lg-6">
			                                            <label>市场价格</label>
			                                            <input name="price" class="form-control" type="text" placeholder="市场价格" value="${product.price/100}">
			                                        </div>
		                                        </div>
		                                        <div class="row">
			                                        <div class="form-group col-lg-6">
			                                            <label>店内价格</label>
			                                            <input name="shopPrice" class="form-control" type="text" placeholder="店内价格" value="${product.shopPrice/100}">
			                                        </div>
			                                        <div class="form-group col-lg-6">
			                                            <label>商品库存</label>
			                                            <input name="quantity" class="form-control" type="text" placeholder="商品库存" value="${product.quantity}">
			                                        </div>
		                                        </div>
		                                        <div class="row">
			                                        <div class="form-group col-lg-6">
			                                           <label>是否记录库存</label>
			                                           <div>
			                                            	<label><input type="radio" value="0" class="i-checks" name="inventoryFlag" <c:if test="${product.inventoryFlag==0}">checked</c:if>>&nbsp;&nbsp;是</label>&nbsp;&nbsp;
			                                            	<label><input type="radio" value="1" class="i-checks" name="inventoryFlag" <c:if test="${product.inventoryFlag==1}">checked</c:if>>&nbsp;&nbsp;否</label>
			                                            </div>
			                                        </div>
			                                        <div class="form-group col-lg-6">
			                                            <label>是否热销</label>
			                                            <div>
			                                            	<label><input type="radio" value="0" class="i-checks" name="hot" <c:if test="${product.hot==0}">checked</c:if>>&nbsp;&nbsp;非热门商品</label>&nbsp;&nbsp;
			                                            	<label><input type="radio" value="1" class="i-checks" name="hot" <c:if test="${product.hot==1}">checked</c:if>>&nbsp;&nbsp;热门商品</label>
			                                            </div>
			                                        </div>
		                                        </div>
		                                        <div class="row">
			                                        <div class="form-group col-lg-6">
			                                            <label>状态</label>${product.productStatus}
			                                            <select name="productStatus" class="form-control">
															<option value="0" <c:if test="${product.productStatus==0}">selected</c:if>>上架</option>
															<option value="1" <c:if test="${product.productStatus==1}">selected</c:if>>下架</option>
														</select>
			                                        </div>
			                                        <div class="form-group col-lg-6">
			                                            <label>商品概要说明</label>
			                                            <input name="generalExplain" class="form-control" type="text" placeholder="商品概要说明" value="${product.generalExplain}">
			                                        </div>
		                                        </div>
		                                        <div class="form-group col-lg-6">
		                                            <label>上传图片</label>
													...
													<div id="uploader-demo">
													    <div id="fileList" class="uploader-list"></div>
															...
															<c:if test="${product.id!=null}">
																<div id="imageFile">
																	<div id="DB_WU_FILE_0" style="float:left" class="file-item thumbnail">
		          														<input type="radio" name="defaultImg" value="DB_WU_FILE_0" checked/>设为默认
		          														<img src="${pageContext.request.contextPath}${product.defaultImg}" style="width: 150px;height: 150px"/>
		          														<div style="padding-top: 2px"><button class="btn btn-default btn-sm" type="button" onclick="deleteImage(this)">删除</button></div>
		          														<input type="hidden" name="DB_WU_FILE_0" value="${product.defaultImg}" />
		     														</div>
		     														<c:forEach items="${product.productImageList}" var="imageInfo" varStatus="status">
																		<div id="DB_WU_FILE_${status.index+1}" style="float:left" class="file-item thumbnail">
			          														<input type="radio" name="defaultImg" value="DB_WU_FILE_${status.index+1}"/>设为默认
			          														<img src="${pageContext.request.contextPath}${imageInfo.url}" style="width: 150px;height: 150px"/>
			          														<div style="padding-top: 2px"><button class="btn btn-default btn-sm" type="button" onclick="deleteImage(this)">删除</button></div>
			          														<input type="hidden" name="DB_WU_FILE_${status.index+1}" value="${imageInfo.url}" />
			     														</div>
																	</c:forEach>
																</div>
															</c:if>
		                                    ...
		                                    <div class="row" style="padding:20px">
		                                         <div class="text-left m-t-xs">
													<button id="saveProductInfo" class="btn btn-success" style="margin-right: 5px;margin-top: 5px"
															type="button">
															<strong>保存</strong>
													</button>
													<button id="back" class="btn btn-success" style="margin-right: 5px;margin-top: 5px"
														type="button">
														<strong>返回</strong>
													</button>
												 </div>
